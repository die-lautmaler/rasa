...
Rasa Open Source 3.0 changed the way how NLU component and dialogue
policies are trained and run during inference. As part of these changes the interfaces
for NLU components and policies were unified and adapted.

The next section outlines which adaptions are required to run your custom NLU components
and policies with 3.0.

#### The `GraphComponent` interface
Any custom NLU components or policies have to inherit the `GraphComponent` interface:

```python (docs/sources/rasa/engine/graph.py)
```

:::tip

It is recommended to use the predefined `rasa.core.policies.policy.Policy` class for
dialogue policies as this provides out-of-the-box behavior for dialogue policies.
:::
...
- `get_default_config` (optional): This should return the default configuration for
   ...
   that there is no default configuration fot the component.
