...
try:
    ...
    logger.debug("Weights & Biases (wandb) is not installed. To use wandb logging, "
...
class WandBLogger:
    ...
    @requires_wandb
    def init_run(self, config: Optional[Dict[Text, Any]] = None, 
    ...
    @requires_wandb
    def log_artifact(self, artifact_path: Text, artifact_name: str, 
...
def create_wandb_logger(config: Optional[Dict[Text, Any]] = None, 
                       run_name: Optional[str] = None) -> Optional[WandBLogger]:
...
def extract_training_metrics(training_data: Any, model_configuration: Any) -> Dict[Text, Any]:
