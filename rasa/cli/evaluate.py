import argparse
from typing import List, TextIO, Text, Int, Dict, Union
...
def _run_markers(args: argparse.Namespace):
    seed = args.seed if "seed" in args else None
    count = args.count if "count" in args else None

    tracker_loader = _create_tracker_loader(args.endpoints, args.strategy, count, seed)
    markers = Markers.from_config(args.config)

    results = _collect_markers(markers, tracker_loader)
    _save_results(args.output_filename, results)
